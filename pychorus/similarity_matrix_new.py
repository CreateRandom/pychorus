...
class SimilarityMatrixNew(object):
    """Abstract class for our time-time and time-lag similarity matrices"""

    __metaclass__ = ABCMeta

    def __init__(self, chroma_a, chroma_b, sample_rate):
        self.chroma_a = chroma_a
        self.chroma_b = chroma_b
        self.sample_rate = sample_rate  # sample_rate of the audio, almost always 22050
        self.matrix = self.compute_similarity_matrix(chroma_a,chroma_b)

    @abstractmethod
    def compute_similarity_matrix(self, chroma_a, chroma_b):
        """"
...
class TimeTimeSimilarityMatrixNew(SimilarityMatrixNew):
    """
    ...
    def compute_similarity_matrix(self, chroma_a,chroma_b):
        ...
        broadcast_x = np.expand_dims(chroma_a, 2)  # (12 x n x 1)
        broadcast_y = np.swapaxes(np.expand_dims(chroma_b, 2), 1,
                                  2)  # (12 x 1 x n)
...
class TimeLagSimilarityMatrixNew(SimilarityMatrixNew):
    """
    Class to hold the time lag similarity matrix where sample (x,y) represents the
    similarity of song frames x and (x-y)
    """

    def compute_similarity_matrix_to_fix(self, chroma_a,chroma_b):
        """Optimized way to compute the time-lag similarity matrix"""
        num_samples_a = chroma_a.shape[1]
        broadcast_x = np.repeat(
            np.expand_dims(chroma_a, 2), num_samples_a + 1, axis=2)

        num_samples_b = chroma_b.shape[1]
        ...
        circulant_y = np.tile(chroma_b, (1, num_samples_b + 1)).reshape(
            12, num_samples_b, num_samples_b + 1)
    ...
    def compute_similarity_matrix(self, chroma_a, chroma_b):
        ...
        num_samples = max(chroma_a.shape[1],chroma_b.shape[1])
        ...
        for i in range(num_samples):
            for j in range(i + 1):
                ...
                time_lag_similarity[j, i] = 1 - (
                    np.linalg.norm(chroma_a[:, i] - chroma_b[:, i - j]) / sqrt(12))
